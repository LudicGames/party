...
import TrackingSystem from '$systems/tracking'
...
export default class TrackingScreen extends Screen {
  ...
  initWorld(){
    ...
    this.engine = new Engine({$app: this.$app})
  }
  ...
  initSystems(){
    ...
    this.cameraSystem = new BaseSystem(true, 5, (delta)=>{
      ...
      this.camera.drawBounds(this.$app.$context, 'blue')
    })
    ...
    this.movementSystem = new MovementSystem(this.$app, this.camera)
    this.engine.addSystem(this.movementSystem)

    this.trackingSystem = new TrackingSystem(this.camera)
    this.engine.addSystem(this.trackingSystem)
  }

  initEntities(){
    // this.platform1 = new Block(0,0,10,10,'blue',this.world, true, -1, false)
